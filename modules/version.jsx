...
const VERSIONS = [
  {
    ...
    current: true,
    changes: [
      { type: 'fix', text: '说明弹窗(共享 billing_help.jsx,4 处同步:商户 代理佣金结算单/代理提款审核 + 代理 佣金结算单/提款审核进度)校正与现流程不符处:①状态数「10 种」→「9 种」(实际只列 9 种,差的「转结下期」已随结算门槛转结机制取消);②主流转链删除已废的「未达门槛→转结下期」句;③财务转结条件「应付≤0」→「应付正负皆可」(状态机终态 + FS 已转结,zh+en);④编号规则补「财务转结单 CF」(归提款审核页)' },
    ],
  },
  {
    ver: 'v3.7.70',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '商户后台 · 代理佣金结算单 · 结算明细抽屉与代理侧对齐:区标题「佣金来源」→「结算金额」;改为单行「分润期号·{period}」+ 金额值(品牌色),删本期佣金/往期转结佣金/可提款金额/转结金额多行(agent_settlement.jsx,与 my_settlement v3.7.69 一致)' },
    ],
  },
  {
    ver: 'v3.7.69',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '专业代理后台 · 佣金结算单 · 结算明细抽屉:区标题「佣金来源」→「结算金额」;行 label「可提款金额」→「分润期号·{period}」(动态取该结算单期号如 W26064),金额值(品牌色)不变(my_settlement.jsx)' },
    ],
  },
  {
    ver: 'v3.7.68',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '专业代理后台 · 佣金结算单 · 提款申请成功弹窗按设计图重做:标题「提款申请已提交」→「提款申请成功」;✓图标放大为实心青绿圆+白勾(64px带阴影);行项重命名(申请单号→提款申请单 / 结算单→佣金结算单「N张」 / 申请金额→申请提款金额),新增「往期财务转结金额」行(负值红,auditCarrySum>0 才显);删「已提交待商户审核」灰条;按钮「知道了」→「确认」居中;新增「查看提款审核进度›」链接→跳转提款审核进度页(mod:my_withdraw_progress)' },
    ],
  },
  {
    ver: 'v3.7.67',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '协作规范落实 HS-INPUT-001(项目文档非产品功能):问题不在规则而在输出模板——立唯一「执行开始固定模板」:动手前只写「【执行开始】依据使用者上一则讯息内容执行」,禁嵌口令原词/任何标识/user* 形式;同步清掉死规则④/⑦/⑨ 正文与速记里残留的字面禁用标识例子,改为中性描述;HANDOFF + 规范两文件同步' },
    ],
  },
  {
    ver: 'v3.7.66',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '专业代理后台 · 佣金结算单 · 申请提款:提交成功提示由 toast 改为弹窗 —— 提交后弹出居中 Modal,显示✓图标 + 申请单号(WR)/ 结算单张数 / 申请金额 / 「已提交,待商户审核」,底部「知道了」关闭(my_settlement.jsx,沿用阻断弹窗同样的 ASTUI.Modal 样式)' },
    ],
  },
  {
    ver: 'v3.7.65',
    ...
    current: false,
    changes: [
      { type: 'add', text: '协作规范新增死规则⑨「[HS-INPUT-001] 输入来源协议」(最高优先级,项目文档非产品功能):只管输入来源不管格式——禁我生成 User/UserA/User[id:*]、虚构消息 ID、模拟使用者回答、预设选项、自动推进;真实使用者任何消息(含光杆口令)都有效、不因格式拒绝或要二次确认' },
      { type: 'remove', text: '正式废止「引用真实 ID / 引用消息 ID / 据以执行:你 mXXXX 的口令」三项(死规则④):我无法取得真实消息 ID,要求引 ID 反而诱发我虚构 ID(m0307/mXXXX)、同属生成使用者标识的违规;执行依据改为「依据使用者上一则讯息内容执行」;同步改自查表④、适用范围、速记;HANDOFF + 规范两文件' },
    ],
  },
  {
    ver: 'v3.7.64',
  ...
  },
  {
    ...
    current: false,
    changes: [
      { type: 'add', text: '专业代理后台 → 提款审核进度 → 付款单弹窗:付款中状态下新增「测试演示用状态改变临时按钮」[付款失败][付款成功],点击调用 APS_BILLING.payFail / pay 切换该笔付款单状态(联动 FS/CS/WR 全链),改完关闭抽屉刷新。仅付款中显示,中/英双语' },
    ],
  },
  {
    ver: 'v3.7.39',
    ...
    current: false,
    changes: [
      { type: 'add', text: '专业代理后台 → 佣金结算单 → 申请提款弹窗:点「申请」新增校验 —— 若该代理存在「未结束的提款审核」(在途状态:审核中/核算中/付款中),阻断并弹出「有未结束提款审核」弹窗(必须等当前提款审核全部结束才能再次发起);无在途则照常生成提款申请单 + 成功提示。中/英双语' },
    ],
  },
  {
    ver: 'v3.7.38',
    ...
    current: false,
    changes: [
      { type: 'add', text: '商户后台 代理提款审核 → 审核模板弹窗 → 「财务核算单 · 财务调整项模板」分组下新增副标题说明:「在此配置财务核算时可选用的调整项。删除某项(如违规扣款)仅对之后的核算生效;已完成的财务核算单不受影响,其已记录的调整项保持原样。」' },
    ],
  },
  {
    ver: 'v3.7.37',
    ...
    current: false,
    changes: [
      { type: 'fix', text: '代理分润报表「總佣金」与「佣金结算单」的佣金逐期对齐 —— 原报表佣金由玩家样本现算(几十~几百),与结算单(W26064 ₹4,880 等)对不上。改为按每期结算单佣金反推玩家样本:報表佣金 = round(settleBase × 5%),令 settleBase = 目标佣金 × 20,5 笔玩家充值/提款/余额聚合后精确等于结算单佣金' },
      { type: 'add', text: '新增共享 window.REVSHARE_PERIOD_TARGET(期 seed → 结算单佣金)+ window.buildTargetFinancials(T)(反推 5 笔玩家财务);商户 代理分润报表(_ARV_buildPeriodRows)与代理后台 分润报表(buildPeriodPlayers)两端共用,12 期数值一致;本期预估期无对应结算单,仍按样本现算' },
    ],
  },
  {
    ver: 'v3.7.36',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '商户后台「代理分润报表」选「每月结算」时,本期预估分润 / 已结算分润 不再显示任何期号 —— 每月已无数据(所有代理均每周),ARV_SETTLED_LIST_MONTHLY 清空、ARV_ESTIMATE_INFO.monthly 置空,两个 tab 改显示空状态「每月结算暂无…分润数据」,不再出现 M2606 / M2605 / M2604 等月度期号' },
      { type: 'fix', text: '配套空值防护:selectedWeek/selectedPeriod/estimateRows/settledRows 及分润方案弹窗的 periodLabel 全部对空月度期做保护,切到每月结算不再因取空对象属性而报错' },
    ],
  },
  {
    ver: 'v3.7.35',
    ...
    current: false,
    changes: [
      { type: 'fix', text: '商户后台「代理分润报表」的已结算分润期数与专业代理后台「分润报表」对齐 —— 原商户端只有 5 期(W26054~W26044),扩充到与代理端一致的 12 期(W26064~W26041),两端期号/周期同步;商户端本期预估也从 W26061 上移到 W26071(避免与已结算期重复)' },
      { type: 'fix', text: '已结算分润「切换期号」下拉过长无法滚动 —— 商户 代理分润报表 + 代理后台 分润报表 两处期号下拉都加 max-height 320 + 纵向滚动' },
    ],
  },
  {
    ver: 'v3.7.34',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '所有代理分润模式统一为「每周分润」:商户后台 代理分润报表的 AGENT_CYCLE_META 四个代理全部改 weekly(原 AC100006 每月 / AC100007 5月切月 / AC100008 6月切周)' },
      { type: 'modify', text: '因所有代理改每周,代理分润报表「每月结算 → 已结算分润」里各代理的已生成结算数据自然清空(月度期仍在,但无任何代理的已结算分润行)。每周/每月切换钮、月度结构、说明等全部保留 —— 仅清空月度已结算数据,不删功能' },
    ],
  },
  {
    ver: 'v3.7.33',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '专业代理后台 → 分润报表 → 已结算分润 的期数与「佣金结算单」同步:由原 5 期(W26054/53/52/51/44)扩充到与 RICH_PERIODS 完全一致的 12 期(W26064→W26041),同期号、同周期、同提款状态,两处不再对不上' },
      { type: 'modify', text: '分润报表「本期预估分润」周期号从 W26061 上移到 W26071(2026/6/29~7/5)—— 因 W26061 已纳入已结算列表(避免同一期同时出现在预估与已结算)' },
      { type: 'modify', text: '佣金结算单佣金金额改合理:原 12 期几乎全为 ₹10000 的假平值,改为 ₹680~₹8100 的差异化真实周佣金;同时保证范例订单状态条件成立 —— 转结下期(W26063 总佣金 ₹680 < 提款门槛 ₹1000)、财务转结(W26052 扣违规/风控/转结后应付 ≤ 0)' },
    ],
  },
  {
    ver: 'v3.7.32',
    ...
    current: false,
    changes: [
      { type: 'remove', text: '商户后台 → 代理账户管理 → 已创建代理 列表删除「代理等级」与「玩家数」两列(表头 + 单元格),精简表格' },
      { type: 'modify', text: '示例代理改名以标明用途:AC範例5 → AC範例5_真實流轉範例;AC範例6 → AC範例6_全狀態範例(agents.jsx 的 agentportal 数组与 ACSamples 两处数据源同步改名,登入账号 loginName 不变)' },
    ],
  },
  {
    ver: 'v3.7.31',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '商户后台「代理分润报表」原独立的「结算周期说明」弹窗(代理分润报表 · 说明)合并进「说明」分页弹窗 —— 撤掉报表上方那颗单独的「结算周期说明」按钮,内容拆成 3 个分页并入:结算周期 / 期编号 / 切换规则,与原有 字段计算 / 分润方案 共 5 个分页' },
      { type: 'add', text: '每月结算相关项全部加「暂不开放」徽章并淡化:结算周期表的「每月结算」行、期编号的「每月结算 M+YY+MM」段、整个「切换规则」分页(每周⇄每月切换涉及每月结算)' },
      { type: 'modify', text: 'ui.jsx FormulaHelp 组件 tabs 新增 node 逃生舱:tab 可传 node 直接渲染自定义 JSX(表格/色块等富排版),优先于 sections —— 供结算周期/期编号/切换规则三页保留原表格与 pill 样式' },
    ],
  },
  {
    ver: 'v3.7.30',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '佣金结算单说明:「主流转链」从原「流转」页移到「订单状态」页顶部(待处理分组上方),状态与流转放一起更连贯' },
      { type: 'modify', text: '佣金结算单说明:原「流转」页(现仅剩业务规则)改名为「业务规则」,并移到「单据流程」左侧 —— 分页顺序变为 业务规则 / 单据流程 / 编号规则 / 订单状态' },
      { type: 'add', text: '提款审核说明(商户 代理提款审核 + 代理 提款审核进度)新增「状态流转」分页:列出 提款申请单 WR(审核中→已通过/已拒绝)、财务核算单 FS(核算中→核算完成/已驳回/已转结)、付款单 PO(付款中→付款成功/付款失败)各自的状态及流向 —— 分页顺序 审核流程 / 单据流程 / 编号规则 / 状态流转' },
    ],
  },
  {
    ver: 'v3.7.29',
    ...
    current: false,
    changes: [
      { type: 'add', text: '提款审核说明(商户 代理提款审核 + 代理 提款审核进度)新增「单据流程」分页 —— 与佣金结算单说明共用同一份 5 层单据链(分润期号→CS→WR→FS→PO),本页单据 WR 高亮' },
      { type: 'modify', text: '编号规则按页拆分:佣金结算单说明只保留 分润期号 + 佣金结算单 CS;提款审核说明只保留 提款申请单 WR / 财务核算单 FS / 付款单 PO,各页只看与本页相关的编号规则' },
      { type: 'modify', text: '佣金结算单说明的「订单状态与流转」拆成两个分页:「订单状态」(10 种状态分组)+「流转」(主流转链 + 业务规则),一次性看太长拆开' },
      { type: 'modify', text: 'billing_help.jsx:单据流程改为内建共享分页(原先由各页 intro 传入,现统一在组件内,新增 flowHi 控制本页单据高亮);最终分页 —— 佣金结算单:单据流程/编号规则/订单状态/流转;提款审核:审核流程/单据流程/编号规则' },
    ],
  },
  {
    ver: 'v3.7.28',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '四个「说明」弹窗内容改为分页(tab 切换),一次性看太长 —— 佣金结算单页(商户 代理佣金结算单 + 代理 佣金结算单)分 3 页:单据流程 / 编号规则 / 订单状态与流转;提款审核页(商户 代理提款审核 + 代理 提款审核进度)分 2 页:审核流程 / 编号规则。下划线式 tab,选中蓝色高亮' },
      { type: 'modify', text: 'billing_help.jsx 共享组件升级为分页容器:新增 intro(各页原有流程图作为第一页)+ introLabel(第一页 tab 标签)两个 prop;tab 标签中英双语;移除原本「append 到底部」的长列表式呈现' },
    ],
  },
  {
    ver: 'v3.7.27',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '去重:「订单状态机与流转」只保留在「佣金结算单」说明(商户 代理佣金结算单 + 代理 佣金结算单),从「提款审核」说明(商户 代理提款审核 + 代理 提款审核进度)移除 —— CS 状态机是「佣金账单」视角,提款审核页本就有 WR→FS→PO 三段流转说明(单据视角),再放一份重复且视角错位。提款审核两页的说明仅保留「编号规则」(该页直接展示 WR/FS/PO 单号,需知编号组成)' },
      { type: 'add', text: 'billing_help.jsx 共享组件加 mode 开关:mode="numbering" 只渲染编号规则(提款审核两页用);默认 full 渲染 编号规则+状态机+流转+业务规则(佣金结算两页用)' },
      { type: 'modify', text: '业务规则文案补充:注明「必须等当前提款审核全部结束(付款成功 / 退回待提款)后,才能再次发起提款审核」,中英同步' },
    ],
  },
  {
    ver: 'v3.7.26',
    ...
    current: false,
    changes: [
      { type: 'add', text: '四个页面的「说明」弹窗统一补充「编号规则」+「订单状态机与流转」—— 商户后台(代理佣金结算单 / 代理提款审核)+ 专业代理后台(佣金结算单 / 提款审核进度):编号规则覆盖 分润期号(W+年月+周序1~4 / M+年月)、CS / WR / FS / PO 五种单据号组成;状态机列出 CS 的 10 种状态(待提款 / 转结下期 / 审核中 / 核算中 / 付款中 / 已拒绝·待提款 / 已驳回·待提款 / 付款失败·待提款 / 财务转结 / 付款成功)分组说明 + 主流转链 + 业务规则(每次申请打包所有待提款,故同一代理不会同时出现多个在途态)' },
      { type: 'add', text: '新增共享组件 modules/billing_help.jsx(window.BillingRulesHelp,单一真源、中英双语),四个弹窗共用,避免四处重复维护;商户端传 lang="zh" 强制中文,代理端跟随顶部语言切换;index.html 在 ui.jsx 后引入' },
    ],
  },
  {
    ver: 'v3.7.25',
    ...
    current: false,
    changes: [
      { type: 'add', text: 'AC100006 演示数据新增 2 个分润期号 W26042(4月第2周)/ W26041(4月第1周),订单状态均为「付款成功」,出现在「佣金结算单」列表' },
      { type: 'feat', text: 'W26043 + W26042 + W26041 三张佣金结算单(CS)合并打包到「同一张」提款申请单(WR)—— data-billing.js 新增 buildMergedPaidChain:一张 WR(结算单数量=3)走完整付款成功链(WR/FS/PO 各一),提款审核进度页显示为一行(数量 3、付款成功),抽屉「提款申请来源 / 关联结算单」列出三张 CS' },
    ],
  },
  {
    ver: 'v3.7.24',
    ...
    current: false,
    changes: [
      { type: 'fix', text: '修正错误分润期号 —— 期号规则为「W+YY+MM+周序」,周序只能 1~4(每月四周)。原 W26050(尾号0)、W26049(尾号9)违规;按其日期归属修正为 4 月对应周:W26050(4/27~5/3)→ W26044(4月第4周)、W26049(4/20~4/26)→ W26043(4月第3周)' },
      { type: 'modify', text: '同步修正 5 处引用:agent_revshare.jsx 已结算周列表、my_revshare.jsx buildSettledPeriodList、data-billing.js 的 PERIODS 与 RICH_PERIODS,确保佣金结算单 / 分润报表 / 提款审核三处期号一致且全部合法' },
    ],
  },
  {
    ver: 'v3.7.23',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '佣金结算单状态分类筛选改为「分组胶囊式」,与提款审核进度页完全一致 —— 商户后台「代理佣金结算单」+ 专业代理后台「佣金结算单」同步:11 个状态按「结算 / 提款申请 / 财务核算 / 付款」四组排列(全部单列),溢出时显示系统默认浅色细滚动条' },
      { type: 'fix', text: '修正上一版做法:原先用下划线 tab + 自定义深色滚动条(带箭头、右侧还出现纵向滚动条),与进度页效果不符;现统一复用进度页 .wp-filter 胶囊样式,选中为胶囊高亮而非下划线' },
    ],
  },
  {
    ver: 'v3.7.22',
    ...
    current: false,
    changes: [
      { type: 'feat', text: '佣金结算单(CS)新增 2 个状态,拆开「提款审核」与「财务核算」共用态 —— 商户后台「代理佣金结算单」+ 专业代理后台「佣金结算单」同步:① 新增「核算中」(WR 已通过、FS 财务核算中),不再跟提款申请「审核中」共用;② 新增「已驳回·待提款」(FS 财务核算驳回),不再跟提款拒绝「已拒绝·待提款」共用,同样退回可重新申请' },
      { type: 'add', text: '订单状态分类 tab 由 9 增至 11:在「已拒绝」右侧加「核算中」,在「财务转结」左侧加「已驳回」(全部/待提款/审核中/转结下期/已拒绝/核算中/已驳回/财务转结/付款中/付款失败/付款成功);两端 TABS + counts 同步' },
      { type: 'modify', text: 'data-billing.js 状态流转改写:approveWR(商户通过)→ CS 转「核算中」;rejectFS(财务驳回)→ CS 转「已驳回·待提款」;rich 演示链 auditing→核算中、fsRejected→已驳回;非 rich 链商户通过后 CS 亦置核算中。createWithdraw 可重新申请范围纳入「已驳回」' },
      { type: 'modify', text: 'BILLING_LABELS.cs 加 auditing(核算中,蓝 b-info)/ auditRejected(已驳回·待提款,红 b-danger);申请提款弹窗、详情抽屉「已退回可重新申请」提示、说明弹窗状态流转文案(中英)同步;withdraw_review CS 弹窗提示一并覆盖 auditRejected' },
    ],
  },
  {
    ver: 'v3.7.21',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '财务调整选 A(所见即所得)+ 不显示未配置项:data-billing.js 去掉所有 FS 自动税金(5%→0,3 处 FS 创建 + approveWR);rich 链 pattern6 财务转结改用风控扣款 110%(原 违规扣款50%+风控60%+税金),违规扣款归零。这样财务核算单「财务调整」区块只会出现已配置的模板项(行政费/风控扣款),应付金额与所列项目一致' },
    ],
  },
  {
    ver: 'v3.7.20',
    ...
    current: false,
    changes: [
      { type: 'feat', text: '商户后台「代理提款审核」接 store 真流转(替换原本页本地假流转 ov):WR 通过→approveWR、WR 拒绝→rejectWR、FS 通过→finishAudit(应付>0 付款中,4 秒后自动 pay 成功)、FS 转结下期→finishAudit(应付≤0 自动转结)、FS 驳回→rejectFS。结果会真同步到「代理佣金结算单」CS 状态、代理后台「我的佣金结算单 / 提款审核进度」(刷新后重置,演示用)' },
      { type: 'modify', text: 'data-billing.js approveWR:生成的 FS 直接为「核算中」(auditing)+ 设核算人/时间 — 按需求取消「待核算」中间态' },
      { type: 'feat', text: '财务调整接 store:保存「编辑 财务调整」时按项目名映射写回 fs 扣款字段(行政费/风控扣款→对应字段,其余→人工调整)并存 _adjustItems 供卡片显示;应付金额由 store 重算,通过/转结用重算后的真金额' },
      { type: 'feat', text: '备注接 store(data-billing.js 新增 getNote/setNote,按单号存):商户后台写的备注跨页保留(刷新重置);首次自动播种一条示例备注' },
      { type: 'remove', text: '移除 withdraw_review 本地假流转相关:ov state、synthFS/synthPO、fsAdj 本地覆盖、notes 本地 state — 全部改读 store,chainOf/derive/timeline/footer 直接读真实单据状态' },
    ],
  },
  {
    ver: 'v3.7.19',
    ...
    current: false,
    changes: [
      { type: 'add', text: '专业代理后台 提款审核进度:① 进度弹窗「关联佣金结算单」② 提款申请单(WR)「提款申请来源」两处的佣金结算单号改蓝色下划线 + 可点击,点击弹出「佣金结算单」CS 弹窗(复用 my_settlement 的 DRow/SrcGroup/ChainRow/CS_TONE_COLOR + ms.* i18n,跟随中英语言),内容含 状态/基本资料/佣金来源/关联提款申请单 + 已拒绝/付款失败/财务转结 提示' },
      { type: 'fix', text: '修语言泄漏:商户后台「代理提款审核」(withdraw_review.jsx)之前用 window.t 跟随代理端 useAgentLang,代理后台切英文时商户这页(列表表头/进度弹窗/CS 弹窗)也跟着变英文。改为强制读中文字典(zh),并移除 useAgentLang 订阅 — 商户后台永远中文,不受代理端语言影响' },
      { type: 'modify', text: '专业代理后台 佣金结算单(my_settlement)/ 提款审核进度(my_withdraw_progress)的「说明」按钮:英文文案由 Guide 改为 Help(与其他页统一),按钮样式由 btn ghost 改为 btn(实心,与其他说明按钮一致)' },
      { type: 'fix', text: '改对文件:代理佣金结算单页自己的 CS 弹窗(agent_settlement.jsx)关联提款申请单下方提示也统一为标准文案「本次提款已结束。相关结算单已退回,可在「佣金结算单」重新申请提款。」— 上一版只改了 withdraw_review 的副本,本页没同步' },
      { type: 'add', text: '佣金结算单弹窗:补 财务转结(auditCarried)状态的下方提示「本次提款已结束。本次财务核算金额已转结到下期财务调整。」(橙色),三处 CS 弹窗 agent_settlement.jsx + withdraw_review.jsx + 代理后台 my_settlement.jsx 全部同步;之前只有 rejected/payFailed 有提示,财务转结缺' },
      { type: 'fix', text: '代理后台 我的佣金结算单 CS 弹窗(my_settlement.jsx)下方提示也统一:已拒绝/付款失败 改标准文案「本次提款已结束。相关结算单已退回...」,去掉旧的「本期佣金已退回待提款」拼接' },
      { type: 'modify', text: '佣金结算单弹窗 → 关联提款申请单 下方提示统一为标准文案(与 提款申请单已拒绝/财务核算已驳回/付款单付款失败 卡片一致),去掉原自定义长句' },
    ],
  },
  {
    ver: 'v3.7.18',
    ...
    current: false,
    changes: [
      { type: 'fix', text: '编辑 财务调整:项目下拉只显示「审核模板·财务调整项模板」中的项(行政费/风控扣款),不再因预填带出非模板项(如 税金);预填时也过滤掉非模板项,避免下拉第二项多出税金' },
    ],
  },
  {
    ver: 'v3.7.17',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '代理提款审核:提款申请单已通过/已拒绝后,底部仅保留「备注」按钮(隐藏 拒绝/通过);财务核算单已通过(付款中/付款成功)或已驳回/已转结后,底部仅保留「备注」按钮(隐藏 财务调整/转结下期/驳回/通过)' },
    ],
  },
  {
    ver: 'v3.7.16',
    ...
    current: false,
    changes: [
      { type: 'add', text: '代理提款审核:补全 CS→提款审核 完整流转(本页本地假流转,不动共享 store)。WR 通过→生成核算单(核算中);WR 拒绝→已拒绝;FS 通过(应付>0)→付款中,延时约 4 秒自动→付款成功;FS 转结下期(应付<0)→已转结;FS 驳回→已驳回。列表三段状态/进度时间轴/WR·FS·PO 单据卡片全部按本地覆盖实时反映' },
      { type: 'modify', text: '本地流转用 ov(按 wr.id)+ 合成 FS/PO 对象驱动 chainOf/derive/buildPhases;不写 data-billing.js,故「代理佣金结算单」与代理后台进度页不同步(按需求只做本页)' },
      { type: 'modify', text: '编辑 财务调整 弹窗:删除「类型」选择;「项目」改为只能从 审核模板·财务调整项模板 下拉选择(保留已存值);取消按钮左侧新增「项目模板管理」按钮(打开 审核模板 弹窗增减项目)' },
    ],
  },
  {
    ver: 'v3.7.15',
    ...
    current: false,
    changes: [
      { type: 'add', text: '代理提款审核 → 财务核算单:「编辑 财务调整」弹窗与卡片关联打通 — 打开时按 fs.id 预填(已保存的自定义调整项 / 否则从 fs 原始扣款字段派生),保存后写入 fsAdj[fs.id]' },
      { type: 'modify', text: '保存财务调整后,卡片「财务调整」区块改显示自定义项;小计 / 核算结果·财务调整 / 应付金额 / 转结金额 实时按新调整项重算(应付=总申请提款+调整合计,应付<0 自动转结)' },
      { type: 'modify', text: '通过/转结下期 的应付金额校验与确认弹窗金额改用重算后的应付金额(fsPayableOf),与财务调整联动' },
    ],
  },
  {
    ver: 'v3.7.14',
    ...
    current: false,
    changes: [
      { type: 'add', text: '代理提款审核 → 财务核算单 子抽屉底部新增 备注 / 财务调整 / 转结下期 / 驳回 / 通过 五个按钮' },
      { type: 'add', text: '通过:校验应付金额>0 → 弹「确认通过财务核算?」(显示应付金额);否则 toast「应付金额小于0,请转结下期」' },
      { type: 'add', text: '转结下期:校验应付金额<0 → 弹「确认转结下期财务核算?」(显示转结金额);否则 toast「应付金额大于0,请通过或驳回审核」' },
      { type: 'add', text: '驳回:弹「驳回申请」(拒绝原因 模板1/模板2/自订义,模板读取 审核模板→财务核算单·已驳回模板)+ 确认 驳回申请(实心红,禁用灰)' },
      { type: 'add', text: '财务调整:弹「编辑 财务调整」(类型 模板/自订 + 项目(模板下拉读 财务调整项模板/自订输入)+ 金额(扣款前面用-)+ 新增项目 + 取消/保存)' },
      { type: 'add', text: '备注:复用通用备注弹窗(noteWR 泛化为 noteTarget,按单据 id 存),转结金额下方新增「备注」区块显示已保存备注' },
      { type: 'modify', text: '通过/驳回/转结/调整/备注 当前为 UI 反馈(toast),实际单据状态流转留待数据关联阶段' },
    ],
  },
  {
    ver: 'v3.7.13',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '代理提款审核 点击结算单号弹出的「佣金结算单」弹窗(进度弹窗关联结算单 + 提款申请单提款来源 两处共用 csView)内容/样式对齐「代理佣金结算单」CS 详情:新增 佣金来源(本期佣金/往期转结)、转结金额、关联提款申请单(WR/FS/PO 链 + 状态)、底部「完整流转时间轴可在提款审核查看」提示;复用全局 AGS_DRow/AGS_SrcGroup/AGS_ChainRow/AGS_CS_TONE_COLOR(agent_settlement.jsx 先于本文件载入)' },
    ],
  },
  {
    ver: 'v3.7.12',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '代理提款审核 → 提款审核进度弹窗 → 关联佣金结算单:结算单号改蓝色加下划线,点击弹出该佣金结算单弹窗(复用 csView)' },
    ],
  },
  {
    ver: 'v3.7.11',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '代理提款审核 三处红色按钮统一为实心红 #dc2626(对齐「停用帐户」确认按钮):①审核模板→确认删除模板「确认」②提款申请单底部「拒绝」(原红描边改实心)③拒绝申请弹窗「确认 拒绝申请」(启用时实心红,禁用态保留浅色)' },
      { type: 'modify', text: '拒绝申请「确认 拒绝申请」禁用态由 btn danger 浅粉色改为灰色(bg-2 底 / line 边 / text-3 字 / not-allowed),去掉 btn danger 类' },
    ],
  },
  {
    ver: 'v3.7.10',
    ...
    current: false,
    changes: [
      { type: 'add', text: '代理提款审核 顶栏「说明」右侧新增「审核模板」按钮(primary)' },
      { type: 'add', text: '「审核模板」弹窗:提款申请单·已拒绝模板(模板1/模板2)、财务核算单·已驳回模板(模板1/模板2)、财务核算单·财务调整项模板(可增删,删除走「确认删除模板?」二次确认);取消/保存' },
      { type: 'modify', text: '提款申请单「拒绝申请」弹窗的 模板1/模板2 改为读取「审核模板」中配置的 提款申请单·已拒绝模板,保存模板后即时联动(原写死 REJECT_TEMPLATES 常量移除)' },
    ],
  },
  {
    ver: 'v3.7.9',
    ...
    current: false,
    changes: [
      { type: 'remove', text: '代理提款审核 → 查看&审核 提款申请单 子抽屉:删除底部「有问题? 联络线上客服」' },
      { type: 'add', text: '提款申请单子抽屉底部新增 备注 / 拒绝 / 通过 三个审核按钮' },
      { type: 'add', text: '合计金额下方新增「备注」区块,显示已保存备注(时间 · 备注人 + 内容);默认 seed 一条示例备注' },
      { type: 'add', text: '「备注」弹窗:备注人(只读)+ 备注输入 + 删除 + 新增 备注 + 取消/保存;保存写入本地 notes(按 WR 单号)' },
      { type: 'add', text: '「拒绝申请」弹窗:拒绝原因 模板1/模板2/自订义 + textarea + 取消/确认拒绝申请' },
      { type: 'add', text: '「确认通过提款申请?」弹窗:显示申请金额 + 取消/确认' },
      { type: 'add', text: '提款申请来源 点击 佣金结算单号 弹出该佣金结算单弹窗(状态/基本资料/佣金 明细)' },
      { type: 'modify', text: '通过/拒绝 当前仅 UI 反馈(toast),实际单据状态流转留待数据关联阶段' },
    ],
  },
  {
    ver: 'v3.7.8',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '代理提款审核 → 提款审核进度弹窗:3 阶段「查看 提款申请单/财务核算单/付款单」改为「查看&审核」(仅本商户页硬编码)' },
      { type: 'remove', text: '代理提款审核 → 提款审核进度弹窗底部「有问题? 联络线上客服」删除' },
    ],
  },
  {
    ver: 'v3.7.7',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '代理提款审核「说明」按钮样式由 btn ghost 改为 btn(实心),与「代理佣金结算单」说明按钮一致' },
      { type: 'modify', text: '代理提款审核 列表操作列「查看」改为「查看&审核」(仅本商户页硬编码,不动代理端共享 i18n)' },
    ],
  },
  {
    ver: 'v3.7.6',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '代理提款审核 页标题由「提款审核进度」改为「代理提款审核」' },
      { type: 'modify', text: '代理提款审核 副标题改为「代理申请提款审核及财务核算出款」' },
      { type: 'add', text: '代理提款审核 列表在「提款单号」左侧新增 代理ID + 代理名称 两列(colSpan 10→12)' },
      { type: 'add', text: '代理提款审核 状态筛选下方新增搜索框(代理ID / 代理名称 / 提款申请单号),输入即过滤列表' },
    ],
  },
  {
    ver: 'v3.7.5',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '商户后台 侧栏 财务 →「提款审核」更名为「代理提款审核」' },
      { type: 'remove', text: '删除原商户「提款审核」页(含商户审核/财务核算/执行付款的旧 WithdrawReviewModule 全部内容)' },
      { type: 'add', text: '商户「代理提款审核」整页复制自「专业代理后台 → 提款审核进度」(my_withdraw_progress.jsx):同列表三段状态、同 3 阶段主抽屉时间轴、同 WR/FS/PO 查看弹窗;withdraw_review.jsx 重建为该页副本,仅模块函数名/window 导出改为 WithdrawReviewModule(各 babel script 独立作用域,helper 不冲突)' },
    ],
  },
  {
    ver: 'v3.7.4',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '商户后台 → 财务 → 代理佣金结算单:范例只保留 AC100006，其余代理的结算单范例不再显示(本页 all 过滤 c.agentId === AC100006);KPI / Tabs 计数 / 列表 / 搜索均随之只统计 AC100006' },
      { type: 'remove', text: '代理佣金结算单 顶栏「导出明细」按钮删除' },
      { type: 'modify', text: '代理佣金结算单「说明」按钮样式由 btn ghost 改为 btn,与「代理分润报表」说明按钮(FormulaHelp)一致' },
    ],
  },
  {
    ver: 'v3.7.3',
    ...
    current: false,
    changes: [
      { type: 'fix', text: '范例两边对得上:data-billing.js 建单的代理名称原取 data.js 随机名(如 AC100006 显示 Sara_Network),与代理后台 useCurrentAgent 取的 curated 名(AC範例6)对不上 — 新增 resolveAgentName() 优先从「商户已创建代理 store」解析,与 useCurrentAgent 同源同逻辑;init/focusAgent/createWithdraw 三处建单均改用解析后的名称' },
      { type: 'fix', text: 'billing store init 开头先调用 window.APS_ensureMerchantAgentsStore() 确保 curated 名(AC範例5~8)就绪,避免直接进商户结算页时 store 未初始化 fallback 到随机名' },
    ],
  },
  {
    ver: 'v3.7.2',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '商户后台 财务 → 代理佣金结算单 整页样式对齐「专业代理后台 → 佣金结算单」:同 4 张 KPI(待申请提款总金额[蓝]/审核中总订单数/审核中总金额/累计已付款总金额,数字用千分位 fmtNum 不再用 K/M 缩写)、同 9 状态 Tabs 组件(原 seg 按钮组)、同清爽 400px 详情抽屉(状态/基本资料/佣金来源/关联提款申请单 分区,替换原 600px 大抽屉)、新增「说明」5 层单据流转弹窗' },
      { type: 'modify', text: '保留商户端 4 处差异:① 列表多 代理ID + 代理名称 两列 ② 多搜索框(代理ID/代理名称/佣金结算单号) ③ 标题副标题不同(代理佣金结算单 · 全代理视角) ④ 数据源为全代理 B.cs(代理后台是 csOf(me)),两边共读 data-billing.js,AC100006 范例天然对得上' },
      { type: 'modify', text: '商户不发起提款:顶栏「申请提款」改为「导出明细」(商户动作),保留「说明」按钮;移除代理端的申请提款弹窗与客服 footer' },
      { type: 'remove', text: '商户结算单页移除 结算时间 日期范围筛选(与代理后台保持一致,代理页本无此项)' },
    ],
  },
  {
    ver: 'v3.7.1',
    ...
    current: false,
    changes: [
      { type: 'modify', text: 'data.js 范例代理总数由 80 收敛到 20(前 5 固定样板 AC範例1~5 + 15 背景占位)— 抽出 AGENT_COUNT=20 常数统一控制' },
      { type: 'fix', text: '修正收敛后的孤儿引用风险:players/codes/cpaRecords/settlements/riskPlayers/logs 原本写死 srand(0,79)(假设代理到 AC100080),全部改为 srand(0, AGENT_COUNT-1),确保所属代理 ID 落在实际存在的 20 个之内,不再指向不存在的代理' },
      { type: 'fix', text: 'dashboard 风控告警卡 写死的越界代理 ID 收敛到 20 内:AC100023 → AC100012、AC100015 → AC100009' },
    ],
  },
  {
    ver: 'v3.7.0',
    ...
    current: false,
    changes: [
      { type: 'feat', text: '佣金结算单(CS)状态体系重构 — 与用户讨论确认:CS 仅「待提款 / 转结下期」为自身状态(创建时定),其余 6 个均映射自 提款审核单(WR)/财务核算单(FS)/付款单(PO)' },
      { type: 'add', text: '拆出两个新 CS 状态:① 财务转结(auditCarried)= 财务核算应付≤0、本期欠款转结下期财务调整(橙 b-orange),区别于「转结下期」= 结算未达门槛自动并入下期佣金(紫);② 付款失败·待提款(payFailed)从「付款中」拆出,退回待提款可重新申请' },
      { type: 'modify', text: '代理后台 佣金结算单 状态分类 tab 由 7 改 9:全部/待提款/审核中(提款审核+财务核算共用)/转结下期/已拒绝(提款拒绝+核算驳回共用)/财务转结/付款中/付款失败/付款成功;「已付款」改名「付款成功」' },
      { type: 'modify', text: '商户后台 财务 → 代理佣金结算单 状态 tab 同步为同一套 9 分类(待申请/审核中/转结下期/已拒绝/财务转结/付款中/付款失败/付款成功),详情抽屉新增财务转结说明' },
      { type: 'modify', text: '付款失败 逻辑(用户选 b):退回待提款、代理可重新申请 — createWithdraw 与申请提款弹窗 eligible 纳入 payFailed;弹窗列表/详情抽屉补「上次付款失败可重新申请」提示' },
      { type: 'modify', text: '演示数据(data-billing.js)对齐:fsCarried→CS 财务转结、付款失败→CS 付款失败;运行时 finishAudit(应付≤0)→auditCarried、payFail→CS payFailed' },
      { type: 'modify', text: '佣金结算单详情「转结金额」扩展:财务转结时取 fs.carryOut 显示(原仅结算门槛转结显示);新增徽章橙色 b-orange tone' },
    ],
  },
  {
    ver: 'v3.6.37',
    ...
    current: false,
    changes: [
      { type: 'add', text: '新增共享 日期范围筛选(agent_common.jsx:window.useDateRange + window.DateRangeBar)— 起止日期输入 + 近14/30/90日/全部 快捷钮' },
      { type: 'modify', text: '商户后台 财务 → 代理佣金结算单 重构对齐设计图:KPI 改为 待申请提款金额 / 待审核订单数 / 待审核金额 / 累计已付金额;状态页签 全部/待申请/审核中/已拒绝/已付款;新增 结算时间 日期范围筛选;列改为 代理ID / 代理名称 / 分润期号 / 结算周期(可排序)/ 结算时间(可排序)/ 佣金结算单号 / 佣金 / 订单状态 / 状态更新时间 / 操作' },
      { type: 'modify', text: '商户后台 财务 → 提款审核 重构为单表(一行一笔提款申请)对齐设计图:分组状态筛选(提款申请/财务核算/付款,复用 wp-filter);新增 提款申请时间 日期范围筛选 + 排序;列 代理ID / 代理名称 / 提款申请单号 / 提款申请时间 / 结算单数量 / 申请金额 / 申请状态 / 核算状态 / 付款状态 / 转结金额(负数红字)/ 付款金额 / 查看&审核;「查看&审核」按当前可操作阶段(WR审核 / FS核算付款)打开对应抽屉,原有审核/核算/付款流程与抽屉保留不变;页面标题改「代理提款审核」' },
    ],
  },
  {
    ver: 'v3.6.36',
    ...
    current: false,
    changes: [
      { type: 'add', text: '(内部)新增共享 useDateRange / DateRangeBar 组件,供财务两页日期筛选复用' },
    ],
  },
  {
    ver: 'v3.6.35',
    ...
    current: false,
    changes: [
      { type: 'add', text: '新增共享表头排序能力(agent_common.jsx:window.useTableSort + window.SortTh)— 点击表头 升序→降序→取消 三态循环,带上下箭头方向高亮' },
      { type: 'add', text: '代理后台 佣金结算单 列表「结算周期」「结算时间」两列加上排序(结算周期按周期起始日、结算时间按结算时间排序)' },
      { type: 'add', text: '代理后台 提款审核进度 列表「提款申请时间」列加上排序' },
    ],
  },
  {
    ver: 'v3.6.34',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '代理后台 提款审核进度 列表「转结金额」金额前加负号(-₹9,400),与单据弹窗内转结金额负值表示一致' },
    ],
  },
  {
    ver: 'v3.6.33',
    ...
    current: false,
    changes: [
      { type: 'add', text: '代理后台 提款审核进度 列表「付款金额」左侧新增「转结金额」列:财务核算应付≤0 转结下期(已转结)时显示该期转结金额(fs.carryOut,红色),其余行显示「—」。表头与空态 colSpan 同步(9→10),中英 i18n(wp.col.carryAmt:转结金额 / Carried Amount)' },
    ],
  },
  {
    ver: 'v3.6.32',
    ...
    current: false,
    changes: [
      { type: 'fix', text: '🐛 代理后台切到 EN 时,侧栏「财务」区段、「提款审核进度」项、以及面包屑、顶栏右上「联系我们」按钮仍显示中文 — 根因:agent_common.jsx i18n 表缺 nav.sec.finance / nav.item.my_withdraw_progress / nav.contact 三个键,T() 回退到中文默认值。补上 EN:Finance / Withdrawal Progress / Contact Us' },
    ],
  },
  {
    ver: 'v3.6.31',
    ...
    current: false,
    changes: [
      { type: 'fix', text: '🐛 代理后台 我的账户 流量来源/安全 等短内容 tab 在手机/平板(≤1023px)下方露出一条灰色带 — 根因:该断点 .app-body 用了 display:block,使 .main 高度只等于内容高度,白底 .content 收缩、下方露出 app 灰底(--bg-0)。改回 .app-body display:flex(侧栏此断点是 fixed 抽屉、不占流,.main flex:1 安全撑满),白底铺到底,灰带消失' },
    ],
  },
  {
    ver: 'v3.6.30',
    ...
    current: false,
    changes: [
      { type: 'fix', text: '🐛 我的账户在手机/窄屏(≤767px、≤479px)最后字段与「编辑」按钮仍被客服悬浮图标遮挡 — 根因:手机媒体查询 .mp-tab-body padding 14/14、12/12 带 !important,把 v3.6.29 加的内联 padding-bottom:96 覆盖回去了,导致窄屏下方只剩 14px、无法滑过 FAB。改为 14/14/96、12/12/96(保留底部 96 滑动空间)' },
    ],
  },
  {
    ver: 'v3.6.29',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '代理后台 我的账户 各 tab(基本资料/权限/流量来源/收款方式/安全等)内容区底部可滑动空间加大(tab-body padding-bottom → 96),避免最后字段被右下角客服悬浮图标遮挡' },
    ],
  },
  {
    ver: 'v3.6.28',
    ...
    current: false,
    changes: [
      { type: 'add', text: '代理后台 Live Chat 客服悬浮图标改为可拖动:按住拖拽即可移动到任意位置(避免遮挡页面按钮),位置写入 localStorage 持久化;未拖动(点击)仍展开/收起客服面板;拖动范围限制在视口内' },
    ],
  },
  {
    ver: 'v3.6.27',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '「联系我们」弹窗(ContactModal)渠道行手机适配:窄屏(≤560px)改 grid 布局 — 左侧图标保留并与 标题/副标题 同行,按钮整宽另起一行(此前误把图标隐藏,现恢复)' },
    ],
  },
  {
    ver: 'v3.6.26',
    ...
    current: false,
    changes: [
      { type: 'add', text: '代理后台 提款审核进度 全模块接入中英 i18n(wp.* 字典):列表表头/状态筛选分组/三段状态文字、提款审核进度弹窗(时间轴各阶段+细项+结束提示)、提款申请单/财务核算单/付款单弹窗(标题·副标题·各分区与字段)、说明弹窗全部支持 EN/中文随语言切换' },
      { type: 'remove', text: '清理 my_withdraw_progress.jsx 内未使用的辅助组件(DocStatus / DocSection / RowLine / KV2 / FsRow)' },
    ],
  },
  {
    ver: 'v3.6.25',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '代理后台 提款审核进度弹窗 标题「提款审核进度」字号加大(18/700),与提款申请单·财务核算单·付款单等单据弹窗标题统一' },
    ],
  },
  {
    ver: 'v3.6.24',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '代理后台 佣金结算单弹窗 / 提款审核进度弹窗 / 提款申请单·财务核算单·付款单弹窗 底部下滑滚动空间加大(body padding-bottom 18→96 / 48→96),避免最后内容被右下角客服悬浮图标遮挡' },
    ],
  },
  {
    ver: 'v3.6.23',
    ...
    current: false,
    changes: [
      { type: 'add', text: '代理后台 佣金结算单 →「申请提款」按钮新增校验:有待提款结算单时弹出申请提款弹窗;没有时弹提示「当前没有待提款订单」(原先无可提款时按钮置灰不可点,改为始终可点+提示)' },
    ],
  },
  {
    ver: 'v3.6.22',
    ...
    current: false,
    changes: [
      { type: 'remove', text: '代理后台 佣金结算单 → 「申请提款」按钮去掉后面的数字提示(原显示 (3) 等可发起提款张数)' },
      { type: 'modify', text: '申请提款弹窗底部汇总(结算单数 / 佣金金额)由居中改为右对齐' },
      { type: 'modify', text: '申请提款弹窗 结算单数 数字改用蓝色(--brand),与佣金金额一致' },
    ],
  },
  {
    ver: 'v3.6.21',
    ...
    current: false,
    changes: [
      { type: 'fix', text: '代理后台 提款审核进度 → 付款单:财务核算完成后(付款中)与付款失败时,「查看付款单」改为可点(原先 paying/payFailed 未生成 PO 导致链接置灰不可点);由 FS 派生一张付款单供查看' },
      { type: 'modify', text: '付款单弹窗内容与样式重构,延续财务核算单弹窗:宽度 400、标题 18/700、蓝色竖条 drawer-sec 分区、单列 label 左/值 右;订单状态改彩色文字(付款中橙 / 付款失败红 / 付款成功绿);付款失败显示「本次提款已结束(退回)」提示' },
      { type: 'modify', text: '付款单弹窗分区改为 状态 / 基本资料(代理名称·代理 ID·付款单号·付款单生成时间·财务单号)/ 收款资料(统一读代理 _payment:收款方式 UPI·IFSC·Account·Real Name·Email)/ 支付商(第三方订单号·付款金额蓝色);去掉原绿色渐变金额框' },
    ],
  },
  {
    ver: 'v3.6.20',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '财务核算单弹窗按提款申请单弹窗样式重构:宽度 400、标题 18/700、分区标题改蓝色竖条 drawer-sec、各行改单列 label 左/值 右、底部留白 48' },
      { type: 'modify', text: '财务核算单 订单状态改为 核算中/已驳回/已转结/核算完成 彩色文字;已驳回显示「原因 + 本次提款已结束(退回)」、已转结显示「本次提款已结束(已转结到下期)」提示,均放在更新时间下方' },
      { type: 'modify', text: '财务核算单 新增「提款申请来源」区(提款审核人 + 关联提款申请单号蓝色 + 金额);财务调整各项仅在有值时显示(行政费/税金/违规扣款/风控扣款/人工补款/保留款)+ 小计;核算结果改无底色行式(总申请提款蓝 / 财务调整红 / 应付金额绿 / 转结金额)' },
    ],
  },
  {
    ver: 'v3.6.19',
    ...
    current: false,
    changes: [
      { type: 'fix', text: '提款申请单弹窗「订单状态」与列表「申请状态」不一致修复:改为按 reviewAt / 关联财务核算单派生 审核中/已通过/已拒绝(原直接读 wr.status 导致已通过的单仍显示审核中)' },
      { type: 'modify', text: '提款申请单弹窗 收款资料 统一读代理收款方式(_payment),收款方式固定显示 UPI、Account 取代理 UPI 账号(不再显示单据自带的 USDT/电汇等)' },
      { type: 'modify', text: '提款申请来源:标题去掉「· N 张」;列表去掉外框,改为无边框分隔线行式' },
      { type: 'modify', text: '提款申请单弹窗底部留白加大(paddingBottom 48),可下拉滚动范围更充足,合计金额不再被底部客服栏遮挡' },
      { type: 'fix', text: '提款申请单弹窗遮罩层级提升(Drawer 新增 elevated:mask/panel z-index 110/111),叠加在「提款审核进度」抽屉之上,背景正确变暗' },
    ],
  },
  {
    ver: 'v3.6.18',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '提款申请单弹窗:拒绝「原因」+「本次提款已结束…」提示从弹窗底部上移到「状态」区更新时间下方(与提款审核进度时间轴一致),底部不再重复显示拒绝原因' },
      { type: 'modify', text: '提款申请单弹窗样式对齐佣金结算单详情:标题字号 18/700、弹窗宽度收窄到 400、分区标题由 uppercase 灰色细标题改为蓝色竖条 drawer-sec 样式' },
    ],
  },
  {
    ver: 'v3.6.17',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '提款申请单弹窗(提款审核进度 → 单据明细)按设计图重构:标题去掉「· WR…」单号(单号下移到基本资料),新增副标题简介「查看本次提款申请的状态、收款资料与佣金来源」;FS/PO 单据同步加副标题' },
      { type: 'modify', text: '提款申请单弹窗内容从 2 列网格改为单列「label 左 / 值 右」行式排版(照图);订单状态由徽章改为彩色文字(审核中橙 / 已拒绝红 / 已通过绿)' },
      { type: 'add', text: '提款申请单弹窗新增独立「收款资料」区块:收款方式 / IFSC / Account / Real Name / Email(从代理 _payment / 申请快照读),不再把收款方式塞在基本资料里' },
      { type: 'modify', text: '提款申请来源列表去掉中间期号,单号(蓝)左 + 金额(蓝)右;合计金额改为绿色、去掉灰底' },
      { type: 'add', text: '提款申请单弹窗底部新增「有问题? 联络线上客服」(点击打开联系我们弹窗),与佣金结算单详情一致' },
    ],
  },
  {
    ver: 'v3.6.16',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '提款审核进度 时间轴「进行中」橙色调亮(#d9820a → #f59e0b),阶段图标与细项▶三角同步' },
      { type: 'modify', text: '提款审核进度 时间轴 未开始阶段(2/3)的编号圆圈边框由虚线改为实线' },
    ],
  },
  {
    ver: 'v3.6.15',
    ...
    current: false,
    changes: [
      { type: 'add', text: '提款审核进度 时间轴「付款失败」下方新增提示:「本次提款已结束。相关结算单已退回,可在「佣金结算单」重新申请提款。」(p3 payFailed note=returned)' },
    ],
  },
  {
    ver: 'v3.6.14',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '提款审核进度「已转结」提示文案:「…本次提款申请金额已转结到下期财务调整。」改为「…本次财务核算金额已转结到下期财务调整。」' },
    ],
  },
  {
    ver: 'v3.6.13',
    ...
    current: false,
    changes: [
      { type: 'add', text: '提款审核进度 时间轴 财务核算「已转结」下方新增提示:「本次提款已结束。本次提款申请金额已转结到下期财务调整。」红框样式同退回提示;note 改为类型(returned/carried)区分两种文案' },
    ],
  },
  {
    ver: 'v3.6.12',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '提款已结束提示文案:「…可在「佣金结算单」重新发起。」改为「…可在「佣金结算单」重新申请提款。」' },
    ],
  },
  {
    ver: 'v3.6.11',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '提款审核进度 主抽屉「本次提款已结束」提示位置上移:由抽屉底部(关联佣金结算单之后)改到时间轴内对应失败「原因」行下方 — 提款审核已拒绝→挂在阶段一原因下,财务核算已驳回→挂在阶段二原因下;红框样式不变' },
      { type: 'modify', text: '提款已结束提示文案精简为「本次提款已结束。相关结算单已退回,可在「佣金结算单」重新发起。」(去掉重复的拒绝原因与「已拒绝/待提款」括注)' },
    ],
  },
  {
    ver: 'v3.6.10',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '佣金结算单页(my_settlement)订单状态徽章去掉前导圆点(csBadge 不再渲染 .dot),与提款审核进度状态徽章保持一致' },
    ],
  },
  {
    ver: 'v3.6.9',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '提款审核进度 列表 申请/核算/付款 三个状态列由纯色文字改为软底 pill 徽章(复用 .badge + b-success/b-warning/b-danger:绿/橙/红),与单据状态徽章风格统一;按要求不带前导圆点(不渲染 .dot)' },
    ],
  },
  {
    ver: 'v3.6.8',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '提款审核进度 时间轴单据链接处理:「查看 财务核算单 / 查看 付款单」始终显示不再隐藏;当对应单据尚未生成(提款审核未通过→无财务核算单 / 财务核算未完成→无付款单)时链接置灰且不可点,生成后才变蓝可点' },
      { type: 'modify', text: '提款审核进度 时间轴「已转结」由橙色改为红色(图标红底⚠ + 红字),与列表筛选/状态列的「已转结」红色保持一致' },
    ],
  },
  {
    ver: 'v3.6.7',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '提款审核进度 时间轴细项文案收敛到规范词,不再自造描述:去掉「· 审核人 X」「· 核算人 X」「· 应付 ₹X」「· ₹金额」「应付≤0,本期欠款转结下期」「等待重新付款」「待核算」「等待商户审核」及 skip 阶段的解释句。细项只用标准状态词:提交申请成功 / 审核中 / 已通过 / 已拒绝 / 核算中 / 已驳回 / 已转结 / 核算完成 / 付款中 / 付款失败 / 付款成功;拒绝·驳回额外保留「原因:…」(唯一允许的非状态词,无图标、红字)' },
    ],
  },
  {
    ver: 'v3.6.6',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '提款审核进度 主抽屉时间轴细节优化:(1)细项时间紧跟文案之后(去掉 flex 撑开,不再贴最右);(2)「进行中」阶段图标由蓝改橙(#d9820a);(3)细项标记由纯色点改为状态对应图标 — 完成绿底✓ / 进行中橙色▶ / 拒绝红底× / 转结橙底! / 待办灰点;(4)「查看 XX单」整段文案改为蓝色(原「查看」前缀为灰);(5)删除「待商户通过后进入财务核算」「待财务核算完成」「待付款」等占位文案,待办阶段仅保留标题 +「查看 XX单」链接' },
    ],
  },
  {
    ver: 'v3.6.5',
    ...
    current: false,
    changes: [
      { type: 'remove', text: '「该期分润方案」弹窗(RevsharePlanModal)删除底部「关闭」按钮 — 右上角 ✕ 已可关闭,底栏冗余;代理后台 分润报表 与 商户后台 分润管理 共用此弹窗,两端同步生效' },
    ],
  },
  {
    ver: 'v3.6.4',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '提款审核进度 进度主抽屉副标题由提款单号(WR…)改为简介「查看本次提款的审核与付款进度」,单号下移至「提款申请金额」区块呈现' },
      { type: 'modify', text: '提款审核进度 主抽屉「提款申请金额」区块优化:去掉状态徽章 + 26px 大号金额 + 「X张结算单·收款方式」说明行,改为单据号 + 合计的行式呈现(与下方「关联佣金结算单」同款式);「关联佣金结算单」标题去掉「· N 张」计数' },
    ],
  },
  {
    ver: 'v3.6.3',
    ...
    current: false,
    changes: [
      { type: 'fix', text: '代理后台 分润报表(my_revshare)分页修正:原页脚「共 N 条 · 第 1 / 1 页」页数写死(始终 1/1)、且表格未真正分页(一次渲染全部行)。改为真分页 — 新增 pageSize 状态、按当前页切片渲染、页码越界自动回退,并替换为统一 Pagination 组件(页数随数据/页大小动态计算 + 上下页 + 条/页切换),本期预估 / 已结算两个 tab 同步生效' },
      { type: 'add', text: '代理后台 提款审核进度(my_withdraw_progress)新增分页页脚:统一 Pagination 组件显示「共 N 条 · 第 X / Y 页」+ 上下页 + 20/50/100 条/页 切换;切换状态筛选时自动回到第 1 页' },
    ],
  },
  {
    ver: 'v3.6.2',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '提款审核进度 演示数据按设计图重排:进度列表固定 8 行,新→旧依次为 审核中 / 已拒绝 / 核算中 / 已驳回 / 已转结 / 付款中 / 付款失败 / 付款成功,每态一行(原本 3 行付款成功 + 顺序不对);RICH_PERIODS 由 12 期精简为 10 期(保留 待提款 / 已转结 两期供「佣金结算单」页演示,不进度列表)' },
      { type: 'modify', text: '提款审核进度页金额去除 K/M 缩写:本页(列表 申请金额 / 付款金额、详情抽屉、WR/FS/PO 单据卡)全部由 F.money 改用 F.fmtNum,显示千分位完整数字(如 ₹10,000、₹9,400),不再缩写为 ₹10.0K;全局 F.money(图表等)不动' },
    ],
  },
  {
    ver: 'v3.6.1',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '提款审核进度 状态筛选重排:「全部」独立成首组(无组名),与「提款申请状态 / 财务核算状态 / 付款状态」三组之间加竖线分隔;组名对齐到各自标签上方、标签行统一基线;改为单行不换行(窄屏横向滚动)' },
      { type: 'add', text: '演示数据补全所有状态:新增 W26050(核算中)/ W26049(已驳回)/ W26048(已转结)/ W26047(付款失败)四期,使每个筛选标签都至少有一条示例(data-billing.js buildSingleChain 新增 auditing / fsRejected / fsCarried / payFailed 四个 stage)' },
    ],
  },
  {
    ver: 'v3.6.0',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '提款审核进度页按设计图重构:状态筛选改为「提款申请 / 财务核算 / 付款」三组分组 pill 标签(组名灰色小标题 + 计数 pill,激活蓝字+蓝底计数);删除原 4 张 KPI 卡片' },
      { type: 'modify', text: '表格改为多状态列:提款单号 / 提款申请时间 / 结算单数量 / 申请金额 / 申请状态 / 核算状态 / 付款状态 / 付款金额 / 操作;申请·核算·付款三状态用彩色文字(进行中橙 / 拒绝失败红 / 完成通过绿),未到阶段显示「—」' },
      { type: 'modify', text: '提款审核进度详情抽屉重构:去掉顶部渐变金额 hero,改为 审核进度时间轴 → 提款申请金额 → 关联佣金结算单 三段(drawer-sec 蓝条标题);宽度 580→460;阶段标题下「查看 提款申请单/财务核算单/付款单」改为蓝字链接;底部新增「有问题? 联络线上客服」(点开联系我们弹窗)' },
      { type: 'add', text: '页面右上「说明」按钮:弹出三段流转(WR→FS→PO)说明弹窗' },
    ],
  },
  {
    ver: 'v3.5.10',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '佣金结算单 页面副标题改为「查看所有已结算佣金订单流转状态及提款申请」' },
    ],
  },
  {
    ver: 'v3.5.9',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '佣金结算单 状态分类由 seg 分段按钮改为下划线式 Tabs(window.UI.Tabs):激活项蓝字 + 蓝色下划线、计数用圆角 pill — 与代理账户管理「全部状态」样式统一' },
      { type: 'remove', text: '佣金结算单 工具栏右侧「近 7 天」时间搜索(DateRange)删除' },
    ],
  },
  {
    ver: 'v3.5.8',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '佣金结算单页 4 个总计 KPI 标题统一加「总」字:待申请提款总金额 / 审核中总订单数 / 审核中总金额 / 累计已付款总金额' },
      { type: 'fix', text: '「审核中总订单数」KPI 数值去掉单位「张」,仅显示数字' },
    ],
  },
  {
    ver: 'v3.5.7',
    ...
    current: false,
    changes: [
      { type: 'fix', text: '申请提款弹窗「编辑收款方式」点击无反应修复:gotoPayment 误把路由对象 {kind,k} 传给 setRoute(应为字符串),现改为 onNav(\'mod:my_profile\') 正确跳转「我的帐户 → 收款方式」tab;同时关闭申请提款弹窗' },
      { type: 'modify', text: '佣金结算单详情抽屉底部「联络线上客服」由原 toast 提示改为弹出「联系我们」弹窗(AgentContactModal);app.jsx 新增 window.APS_OPEN_CONTACT 全局开关供调用' },
    ],
  },
  {
    ver: 'v3.5.6',
    ...
    current: false,
    changes: [
      { type: 'fix', text: '修正版本号规则:之前误将 v3.5.3 跳号到 v3.6.0/v3.6.1,现回改为 v3.5.4/v3.5.5(只动第三位小版本号)' },
      { type: 'modify', text: '佣金结算单详情抽屉宽度由 460 收窄到 400,减小 label-值之间过大的中间空白' },
      { type: 'modify', text: '抽屉底部「有问题? 联络线上客服」改为左对齐' },
    ],
  },
  {
    ver: 'v3.5.5',
    ...
    current: false,
    changes: [
      { type: 'fix', text: '佣金结算单英文模式下日期格式统一为 年/月/日 时:分:秒(与中文一致),不再显示 en-GB 的 日/月/年 格式' },
    ],
  },
  {
    ver: 'v3.5.4',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '佣金结算单详情抽屉标题「佣金结算单」字号加大(18px 粗体)' },
      { type: 'modify', text: '详情抽屉副标题由结算单号改为简介文字(本期佣金结算明细与提款流转状态);结算单号仍在「基本资料」展示' },
      { type: 'modify', text: '详情抽屉「基本资料」中结算时间与结算周期位置对调(现为 结算周期 在上、结算时间 在下)' },
      { type: 'add', text: '佣金结算单页面 + 详情抽屉 + 申请提款/说明弹窗 全量接入中/英 i18n(ms.* 翻译键):KPI/页签/表头/状态标签(CS/WR/FS/PO)/字段/按钮/toast 均随顶部语言切换;日期按语言本地化' },
    ],
  },
  {
    ver: 'v3.5.3',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '「转结下期」状态颜色由中性灰改为紫色(新增 .b-purple 徽章 + CS_TONE_COLOR),表格徽章与详情抽屉状态文字同步' },
      { type: 'modify', text: '佣金结算单详情抽屉分区标题(状态/基本资料/佣金来源)改为更醒目的 .drawer-sec 样式:14px 粗体深色 + 蓝色左端色条,区隔更明显' },
      { type: 'modify', text: '佣金结算单详情抽屉金额去掉 K/M 缩写,改为完整数字千位分隔(F.fmtNum):本期佣金 ₹1,900 / 可提款金额 ₹2,000 等' },
    ],
  },
  {
    ver: 'v3.5.2',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '佣金结算单「查看」详情抽屉按设计图重构:去掉顶部 hero 金额渐变框;标题「佣金结算单」+ 副标题结算单号;结构改为 状态 / 基本资料 / 佣金来源 三区块,均为 label-左 value-右 行' },
      { type: 'modify', text: '佣金来源改为按来源 CS 分组:本期佣金挂在本期 CS 单号下、往期转结佣金挂在来源期 CS 单号下(灰字小标题);下方可提款金额(蓝)+ 转结金额(红);去掉 CPA/RevShare 拆分' },
      { type: 'modify', text: '抽屉底部改为「有问题? 联络线上客服」,移除下载/复制/申请提款按钮(提款仍可从页面右上发起);关联提款申请单(WR/FS/PO)区块仅在已发起提款时显示' },
      { type: 'add', text: '演示数据打通转结链:W26063(转结下期 ₹100)转结并入 W26064 → W26064 本期佣金 ₹1900 + 往期转结 ₹100(来源 CSW26063000017)= 可提款 ₹2000' },
    ],
  },
  {
    ver: 'v3.5.1',
    ...
    current: false,
    changes: [
      { type: 'fix', text: '修正佣金结算单演示数据「分润期号」与「结算周期」对不上:期号编码=W+年+月+月内周序,之前把 6 月期号(W2606x)错配成了 5 月日期。现按全站约定修正:W26064→2026/6/22~6/28、W26063→6/15~6/21、W26062→6/8~6/14、W26061→6/1~6/7、W26054→5/25~5/31、W26053→5/18~5/24、W26052→5/11~5/17、W26051→5/4~5/10' },
    ],
  },
  {
    ver: 'v3.5.0',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '代理后台侧栏「结算单」重命名为「佣金结算单」(nav.item.my_settlement i18n 值)' },
      { type: 'add', text: 'CS 新增「付款中」(paying)状态(品红 b-magenta 徽章):财务核算完成进入付款、尚未生成 PO 时 CS 显示「付款中」;「已转结」改标为「转结下期」' },
      { type: 'modify', text: '佣金结算单页签扩为 7 个:全部/待提款/转结下期/审核中/付款中/已拒绝/已付款' },
      { type: 'modify', text: '重写登入代理演示数据(buildRichDemo):8 期示例,仅最新一期为待提款,旧期必定进入下游状态(因每次提款会打包所有待提款单);顺序覆盖 待提款/转结下期/审核中/付款中/已拒绝/已付款×3' },
      { type: 'modify', text: '佣金结算单 KPI 重命名:待审核订单数→审核中订单数、待审核金额→审核中金额、累计已付金额→累计已付款金额' },
      { type: 'add', text: '全站分页器新增「每页条数」选择框(20/50/100 条/页,默认 20):ui.jsx Pagination 加 onPageSize/pageSizeOptions;推广到 13 个表格(佣金结算单/CPA报表/分润报表/我的玩家/我的Code/Code管理/代理收益 + 商户后台 代理结算单/提款审核/代理账户/玩家/Code/CPA)' },
    ],
  },
  {
    ver: 'v3.4.3',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '佣金结算单订单状态「已拒绝」改标为「已拒绝·待提款」(BILLING_LABELS.cs.rejected.label),同步反映在订单状态徽章与详情抽屉;表明提款被拒后佣金已退回可重新申请。筛选页签仍保留短标「已拒绝」' },
    ],
  },
  {
    ver: 'v3.4.2',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '佣金结算单「申请提款」弹窗按设计图重构:副标题改为「申请提款所有待提款结算单」;收款方式下拉选择 → 只读收款资料卡(收款方式/IFSC/Account/Real Name/Email,与「我的帐户→收款方式」同源 me._payment)' },
      { type: 'add', text: '申请提款弹窗右上「编辑收款方式」蓝字链接:设 window.__AGENT_PROFILE_TAB=payment 并 onNav 跳转「我的帐户」并直接定位到「收款方式」tab;AgentProfileModule 初始 tab 支持外部预设' },
      { type: 'modify', text: '提款改为 PRD「自动关联所有可提款结算单」:去掉逐张勾选框/全选,待提款结算订单改为只读列表;底部汇总改为居中「结算单数 N · 佣金金额 ₹X」;app.jsx 给 MySettlementModule 传 onNav' },
    ],
  },
  {
    ver: 'v3.4.1',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '佣金结算单按设计图对齐:操作栏去掉内嵌「申请提款/重新申请」按钮,每行只保留「查看」蓝字链接(新增 .link-act 样式)' },
      { type: 'remove', text: '佣金结算单 4 个 KPI 总数框去掉下方提示小灰字(如「X 张可提 · 点右上申请」),仅留标题+数字,与设计图一致' },
    ],
  },
  {
    ver: 'v3.4.0',
    ...
    current: false,
    changes: [
      { type: 'modify', text: '代理后台「佣金结算单」(my_settlement)按 A 版设计稿重构:KPI 改为 待申请提款金额/待审核订单数/待审核金额/累计已付金额;状态页签改为 6 个(全部/待提款/已转结/审核中/已付款/已拒绝),默认「全部」;列表列序对齐设计稿(分润期号/佣金/结算单号/结算周期/结算时间/订单状态/状态更新时间/操作)' },
      { type: 'modify', text: 'my_settlement 详情抽屉重构为 状态/基本资料/佣金来源/关联提款申请单 四区块,关联提款申请单号(WR/FS/PO)突出展示;新增「说明」按钮弹窗讲解 5 层单据流转' },
      { type: 'add', text: 'CS 新增「已拒绝」状态(BILLING_LABELS.cs.rejected):提款申请/财务核算被拒后 CS 退回「已拒绝」(保留关联 WR 单号以便回溯),可重新发起提款;createWithdraw 同时受理 待提款+已拒绝;data-billing 为登入代理生成一条 rejected 演示链' },
      { type: 'modify', text: '代理后台「提款审核进度」(my_withdraw_progress)时间轴改为 3 阶段嵌套结构(提款审核申请→财务核算→付款单),每阶段含子状态(提交成功/审核中/已通过/已拒绝;核算中/核算完成/已转结/已拒绝;付款中/付款成功/付款失败)及「查看单据」入口' },
      { type: 'add', text: 'my_withdraw_progress 新增 WR/FS/PO 单据明细子抽屉(WRCard/FSCard/POCard),对齐设计稿:提款申请单(来源CS+合计)、财务核算单(财务调整明细+核算结果)、付款单(收款资料+付款金额)' },
    ],
  },
  {
    ver: 'v3.3.0',
    ...
    current: false,
    changes: [
      { type: 'feat', text: '佣金结算与提款流程重构为 5 层单据链:分润期号(W/M)→ 佣金结算单(CS)→ 提款申请单(WR)→ 财务核算单(FS)→ 付款单(PO)。新增共享单据 store data-billing.js(window.APS_BILLING + useBilling hook),商户端与代理端共读同一份数据,任一端操作即时同步' },
      { type: 'feat', text: '单号规则完全照 PRD 落地:CSW26054000001(CS+期号+6位流水)/ WR20260603000001(WR+申请日期+6位)/ FS+建立日期+6位 / PO+付款日期+6位' },
      { type: 'modify', text: '代理后台「佣金结算单」(my_settlement)改造:单号改 CSW…;新增字段 本期佣金/上期转结佣金/本期总佣金/最低提款门槛/本期可提金额;状态改为 待提款/已转结/审核中/已付款;详情抽屉展示佣金构成 + 提款资格 + 关联提款链(WR/FS/PO);申请提款生成 WR(打包 1~N 张 CS)' },
      { type: 'add', text: '代理后台 财务 新增「提款审核进度」(my_withdraw_progress, P0-9):WR 列表 + 4 阶段时间轴(提交提款申请 → 商户审核 → 财务核算 → 付款完成),展示关联 CS、FS 核算扣款明细、PO 付款单' },
      { type: 'add', text: '商户后台 财务 新增「代理佣金结算单」(agent_settlement, P0-8):查看全代理 CS,按期号/状态/代理筛选,详情展示佣金构成 + 关联提款链' },
      { type: 'modify', text: '商户后台「提款审核」(withdraw_review)升级为三层单据流:WR Tab(商户审核 通过→生成FS / 拒绝→CS退回待提款)、FS Tab(财务核算:行政费/税金/违规扣款/风控扣款/人工调整/保留款 实时算应付,开始核算→核算中→完成核算→付款中/已转结,执行付款→生成PO,支持驳回/付款失败重试)、PO Tab(付款流水只读)' },
      { type: 'modify', text: 'PRD P0-8 改为「佣金结算单生成与审核」(共用,mapping 商户 agent_settlement + 代理 my_settlement);P0-9 改为「提款申请与财务核算付款」(共用,mapping 商户 withdraw_review + 代理 my_withdraw_progress),scope 同步更新为 WR/FS/PO 单据职责' },
    ],
  },
  {
    ver: 'v3.2.75',
