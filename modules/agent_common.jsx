...
(function() {
  ...
  add('rv.formula.period',
    'STEP-1-1:計算本期所有用戶總合行為判斷本期實際消耗了多少歷史資產\n    (總上期期末餘額) + (總本期充值 - 總本期提現 - 總本期期末餘額) = 總本期變動佣金基數\n\nSTEP-1-2:計算本期所有用戶總合行為判斷平台是否虧損的基數\n    總本期變動佣金基數 + (總上期結算佣金基數) = 總本期結算佣金基數\n\nSTEP-2:校驗本期平台是盈利或虧損 / 持平\n    2-1 總本期結算佣金基數 ≤ 0,代表平台虧損或持平,不計算佣金,接 STEP-4\n    2-2 總本期結算佣金基數 > 0,代表平台盈利,計算佣金,接 STEP-3、STEP-4\n\nSTEP-3:計算本期佣金\n    總本期結算佣金基數 × 佣金分潤比例 = 本期佣金\n\nSTEP-4:本期帶入下期值\n    · 總本期期末餘額\n    · 總本期結算佣金基數\n      (如果總本期結算佣金基數是負值則帶入下期該負值;如果為正值則將 0 帶入下期)',
    'STEP-1-1: Calculate this period\'s total user behavior to determine actual consumption of historical assets\n    (Total Prev Ending Balance) + (Total Deposits - Total Withdrawals - Total Ending Balance) = Total Period Variable Commission Base\n\nSTEP-1-2: Add prev settled commission base to determine whether the platform is at a loss\n    Total Period Variable Commission Base + (Total Prev Settled Commission Base) = Total Period Settled Commission Base\n\nSTEP-2: Verify whether the platform profits or loses this period\n    2-1 Total Period Settled Commission Base ≤ 0 — Platform at loss or break-even, no commission, go to STEP-4\n    2-2 Total Period Settled Commission Base > 0 — Platform profits, calculate commission, then STEP-3 & STEP-4\n\nSTEP-3: Calculate this period\'s commission\n    Total Period Settled Commission Base × RevShare Ratio = This Period Commission\n\nSTEP-4: Values carried to next period\n    · Total Period Ending Balance\n    · Total Period Settled Commission Base\n      (Negative values carry over to next period as-is; positive values carry 0)');
