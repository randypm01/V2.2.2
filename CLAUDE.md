> ⚠️⚠️ 开局第一件事:先 `read_file HANDOFF.md` —— 它是「当前真实快照 + 生成流程 SOP + 熔断/死规则 + 待办」,**优先级高于本文件**。本文件(CLAUDE.md)是早期 P0 精简版,文件清单/范围已过时,只取其规则与偏好,其余以 HANDOFF.md + `ls modules/` + `index.html` 为准。

---

# 博彩平台_專業代理後台系統 v1.0(P0 精簡版)— 项目记忆

> 用于在新对话中无缝接续。新 chat 开始时,Claude 会先读 HANDOFF.md,再参考这份文件。
...
**每次动手改代码前,先用一两句话报「要改哪几个文件 / 函数」,等用户确认方向后再开工。** 用户多次强调生成时间要压在 5~10 分钟内,核心靠以下习惯:

1. **先报改动范围再动手** — 动代码前先一句话列「改 X 文件的 Y 函数」,让用户确认,避免做完发现方向错了返工。
2. **只读相关文件 + grep 精准定位** — 不整文件通读;无关文件一律不读(读大文件是最大的时间黑洞)。
3. **并行调用工具** — 互不依赖的读取/搜索一次发出,不逐个等。
4. **少做防御性验证** — 改完只跑 1 次关键 eval/截图确认,其余交给后台 verifier,不反复自检。
5. **范围严格按用户说的来** — 不自作主张扩大改动 / 重构(历史教训:「每月分润」删过头又还原,白白多花两轮)。
6. **及时 snip 已结束的探索上下文** — 上下文越精简,响应越快。
7. **纯文案 / 纯样式的小改 → 不自检**:改完 + 记版本即交付,不跑 show_html、不跑 eval(静态文字/CSS 不可能崩,自检纯属浪费时间);验证交给后台 verifier。只有涉及 JS 逻辑/数据/交互的改动才跑 1 次关键验证。
8. **报「改哪个文件」前先 grep 确认**,不靠记忆猜文件(历史教训:加副标题时先猜 agents.jsx,实际在 withdraw_review.jsx)。
...
代理后台需要登入才能验证。**统一用 `AC100006 · rajeshmedia / 密码 Test@1234`**(快速选择列表第 AC範例6 条)。
- 所有「快速选择已创建账户」里的账号密码都是 `Test@1234`(不是 123456)
- 不要随便挑账号 —— 有些是被停用/冻结的,登不进或进去看不到数据
- 驱动登入:填账号 `rajeshmedia` + 密码 `Test@1234` → 点弹窗内最后一个「登入」按钮 → `window.CURRENT_AGENT_ID` 有值即成功 → `window.goRoute('mod:xxx')` 跳页
